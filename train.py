...
data_path = "../data/point" # todo 数据集路径
# 注： 执行之前请先划分数据集
# 超参数设置
params = {
    ...
    'device': device,  # 设备
    'lr': 1e-3,  # 学习率 1e-3
    'batch_size': 128,  # 批次大小 4
    'num_workers': 0,  # 进程
    'epochs': 10,  # 轮数
    "save_dir": "../checkpoints/",  # todo 保存路径
    "pretrained": True,
    "num_classes": 10, # len(os.listdir(osp.join(data_path, "train"))),  # 类别数目, 自适应获取类别数目
...
class SELFMODEL(nn.Module):
    def __init__(self, model_name=params['model'], out_features=params['num_classes'],
                 pretrained=True):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=6, out_channels=3, kernel_size=1, padding=0)
        self.model = timm.create_model(model_name, pretrained=pretrained)  # 从预训练的库中加载模型
        self.GE = GatherExcite(6)
...
def train(train_loader, model, criterion, optimizer, epoch, params):
    ...
    for i, (images, target, path, std) in enumerate(stream, start=1):  # 开始训练
        # print(images.shape)
        # print(target.shape)
        # print(images, target)
...
def validate(val_loader, model, criterion, epoch, params):
    ...
    with torch.no_grad():  # 开始推理
        for i, (images, target, path, std) in enumerate(stream, start=1):
...
if __name__ == '__main__':
    ...
    valid_dataset = MyDataset(mode='val')
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=params['lr'], weight_decay=params['weight_decay'])  # 设置优化器
    # 损失函数和优化器可以自行设置修改。
    # criterion = nn.CrossEntropyLoss().to(params['device'])  # 设置损失函数
    # optimizer = torch.optim.AdamW(model.parameters(), lr=params['lr'], weight_decay=params['weight_decay'])  # 设置优化器
